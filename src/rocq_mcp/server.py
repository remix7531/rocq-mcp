...
async def rocq_step_multi(
    ...
    Each entry carries ``success``; successes add ``proof_finished``,
    the first entry per distinct outcome adds ``goals``/``focus_depth``
    (repeats carry ``same_outcome_as``), failed tactics carry ``error``
    (and ``feedback`` when a tactic prints).  Advance a
...
# cost.  Content ships as package data: rocq_mcp/guide_<name>.md.


def _read_guide(name: str) -> str:
    return (Path(__file__).parent / f"guide_{name}.md").read_text(encoding="utf-8")
...
4. Explore with `rocq_step_multi(from_state=s, tactics=[...])`.  Start
   with an automation battery: ["trivial.", "reflexivity.",
