"""Tests for the A8 bullet / focus-management payload of ``rocq_check``.
...
async def test_open_brace_adds_focus_payload(mock_pytanque, reset_state):
    ...
    focus frame onto the stack.  All five A8 keys must appear with the
    expected values."""
    ...
    after_resp = SimpleNamespace(
        goals=[g1], stack=[([], [g2])], shelf=[], given_up=[]
    )
...
async def test_dash_bullet_payload(mock_pytanque, reset_state):
    """A ``-`` bullet should also get the A8 payload (no stack change,
...
async def test_non_bullet_body_unaffected(mock_pytanque, reset_state):
    """Plain tactic bodies must NOT gain any A8 keys."""
    import rocq_mcp.server
    import rocq_mcp.interactive as _interactive
