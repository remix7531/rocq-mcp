...
`rocq_notations`, `rocq_goal_at`, `rocq_hover`) require the `pet`
...
```
feat(step_multi): per-tactic timeouts and time_ms in results
fix(compile_file): warn about stale .vo dependencies
